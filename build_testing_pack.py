#!/usr/bin/env python3
"""
Build 'The Army Approach to Individual Training' as a branded NZ Army .docx.

This is the authoritative statement of the approach, not the working synthesis
that produced it. It states, applies and assures; it does not propose.

The body comes from SOURCE_FILE. The statement and the model are drawn from the
shared source (APPROACH_FILE), the same source the A3 poster and the directive
build from, so the authority and the products that carry it cannot disagree.

Bracketed text is rendered in Army Red: every gap the issuing staff must
complete should be visible at a glance.

    python3 build_training_approach.py
    /usr/bin/python3 convert_to_pdf.py output/individual-training-approach.docx \\
        output/individual-training-approach.pdf
"""
...
# ----------------------------------------------------------------- CONFIG ---
SOURCE_FILE        = "./testing-the-approach.md"
LOGO_FILE          = "./assets/nz-army-logo.png"
OUTPUT_DOCX        = "./output/testing-the-approach.docx"
PROTECTIVE_MARKING = "UNCLASSIFIED"

TITLE       = "Testing the Approach"
SUBTITLE    = "A falsification pack for the individual training and AI models"
FOOTER_LEFT = "Testing the Approach"
FOOTER_REF  = "For return"
...
HEADER_FIELDS = [
    ...
    ("Date", "September 2026"),
    ("Return by", "[date]"),
    ("Purpose", "To find where these models fail"),
]
...
while i < len(lines):
    ...
    if s.startswith("# ") or s.startswith("**A falsification"):

#!/usr/bin/env python3
"""
Build 'AI and Army Operational Effectiveness' as a branded NZ Army .docx
staff paper: a first-principles strategic analysis for senior Army leadership.

Same brand system and markdown conventions as the approach document and the
directive. Bracketed text renders in Army Red as detail to be completed.

    python3 build_ai_effectiveness.py
    /usr/bin/python3 convert_to_pdf.py output/ai-army-operational-effectiveness.docx \\
        output/ai-army-operational-effectiveness.pdf
"""
...
OUTPUT_DOCX        = "./output/ai-army-operational-effectiveness.docx"
...
def add_table(rows):
    ...
    for r, row in enumerate(rows):
        # a content table: rows size to their text and stay on one page
        ...
        for c, cell_text in enumerate(row):
            ...
            if r < len(rows) - 1:
                p.paragraph_format.keep_with_next = True   # whole table together
            if r == 0:
...
while i < len(lines):
    ...
    if s.startswith("# ") or s.startswith("**A first-principles"):
